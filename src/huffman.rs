pub mod huffman_encoding {
    ...
    mod huffman_tree {
        ...
        pub(crate) fn print_frequency_table(
            frequency_table: &Vec<u32>,
            as_char: bool,
            verbose: bool,
        ) {
    ...
    fn print_huffman_encoding(huffman_encoding: &HashMap<u8, String>, as_char: bool) {
        let mut symbols = Vec::from_iter(huffman_encoding.iter());
        symbols.sort();
    ...
    pub fn encode_string(characters: &String, string_to_encode: &String) {
        ...
        let huffman_encoding = self::generate_huffman_encoding(&huffman_tree);
        println!("");
        self::print_huffman_encoding(&huffman_encoding, true);
        println!("");

        // contents
        let mut output_buffer: String = String::from("");
        for character in string_to_encode.chars() {
            let code_word = match huffman_encoding.get(&(character as u8)) {
    ...
    pub fn encode_file(
        ...
        if verbose {
            ...
            println!("");
            self::print_huffman_encoding(&huffman_encoding, !binary);
            println!("");
        }
        ...
        if !binary {
            ...
            for character in file_contents.chars() {
                ...
                let code_word = huffman_encoding.get(&symbol).unwrap();
                output_buffer.push_str(code_word);
